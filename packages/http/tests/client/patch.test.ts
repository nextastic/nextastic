...
it('should successfully make a PATCH request', async () => {
  const data = { foo: 'bar' }
  const response = await http.patch('https://api.example.com/data', data)
...
it('should handle FormData in PATCH requests', async () => {
  const formData = new FormData()
  formData.append('file', new Blob(['test']))

  await http.patch('https://api.example.com/upload', formData)
...
it('should handle 401 unauthorized responses', async () => {
  ...
  await expect(
    http.patch('https://api.example.com/data', { foo: 'bar' }),
...
it('should handle invalid JSON responses', async () => {
  ...
  const response = await http.patch('https://api.example.com/data', {
    foo: 'bar',
  })
...
it('should pass custom headers in the request', async () => {
  const data = { foo: 'bar' }
  await http.patch('https://api.example.com/data', data, {
    headers: {
