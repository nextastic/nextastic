...
it('should successfully make a DELETE request', async () => {
  const response = await http.delete('https://api.example.com/data')
...
it('should handle 401 unauthorized responses', async () => {
  ...
  await expect(http.delete('https://api.example.com/data')).rejects.toThrow(
    'Unauthenticated',
  )
})
...
it('should pass custom headers in the request', async () => {
  await http.delete('https://api.example.com/data', {
    headers: {
